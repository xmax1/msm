...
def validate_config(cfg):
    ...
    assert cfg.log_root is not None, "log_root cannot be None."


def read_configs(config_path, base_config_path=None):
...
def scan(cell, inputs, use_obs, initial):
    ...
    inputs = tf.nest.map_structure(  # applys this function to every entry in inputs (ie if inputs was a dict)
        lambda x: tf.transpose(x, [1, 0] + list(range(2, len(x.shape.as_list())))),
        inputs,
    )
    outputs = tf.scan(  # previous_output, current_input (initial = agg, is zero for the first loop. Cell takes in the state and the input along first dim)

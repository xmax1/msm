...
class Encoder(tools.Module):
    ...
    def __init__(self, levels, tmp_abs_factor, dense_layers=3, embed_size=100, channels_mult=1, var_scope='encoder_convdense'):
        """
        Arguments:
            obs : Tensor
        ...
        assert tmp_abs_factor >= 1, 'tmp_abs_factor should be >=1, found {}'.format(tmp_abs_factor)
        assert not dense_layers or embed_size, 'embed_size={} invalid for Dense layer'.format(embed_size)
    ...
    def __call__(self, obs):
        ...
        with tf.name_scope(self._var_scope):
            ...
            filters = 32
            hidden = self.get('h1_conv', tfkl.Conv2D, self._channels_mult*filters, 4, **self._kwargs)(hidden)
            hidden = self.get('h2_conv', tfkl.Conv2D, self._channels_mult*filters*2, 4, **self._kwargs)(hidden)
            hidden = self.get('h3_conv', tfkl.Conv2D, self._channels_mult*filters*4, 4, **self._kwargs)(hidden)
            hidden = self.get('h4_conv', tfkl.Conv2D, self._channels_mult*filters*8, 4, **self._kwargs)(hidden)
            hidden = tf.layers.flatten(hidden) # shape: (BxT, :)
            hidden = tf.reshape( hidden, tf.concat([tf.shape(obs)[:2], [hidden.shape.as_list()[-1]]], -1) ) # shape: (B, T, :)
            layer = hidden
            ...
            for level in range(1, self._levels):
                for i_dl in range(self._dense_layers-1):
                    hidden = self.get('h{}_dense'.format(5+(level-1)*self._dense_layers+i_dl), tfkl.Dense, self._embed_size, activation=tf.nn.relu)(hidden)
                if self._dense_layers > 0:
                    hidden = self.get('h{}_dense'.format(4+level*self._dense_layers), tfkl.Dense, feat_size, activation=None)(hidden)
                layer = hidden

                timesteps_to_merge = np.power(self._tmp_abs_factor, level)
                # Padding the time dimension.
                timesteps_to_pad = tf.mod( timesteps_to_merge - tf.mod(tf.shape(layer)[1], timesteps_to_merge), timesteps_to_merge )
                ...
                layer = tf.reshape( layer, [tf.shape(layer)[0], -1, timesteps_to_merge, layer.shape.as_list()[2]] )
...
class Decoder(tools.Module):
    ...
    def __call__(self, states):
        ...
        """
        with tf.name_scope(self._var_scope):
            hidden = self.get('h1', tfkl.Dense, self._channels_mult*1024, None)(states) # (B, T, 1024)

            # Squeezing batch and time dimensions, and expanding two extra dims.
            hidden = tf.reshape(hidden, [-1, 1, 1, hidden.shape[-1].value]) # (BxT, 1, 1, 1024)

            filters = 32
            hidden = self.get('h2', tfkl.Conv2DTranspose, self._channels_mult*filters*4, 5, **self._kwargs)(hidden) # (BxT, 5, 5, 128)
            hidden = self.get('h3', tfkl.Conv2DTranspose, self._channels_mult*filters*2, 5, **self._kwargs)(hidden) # (BxT, 13, 13, 64)
            hidden = self.get('h4', tfkl.Conv2DTranspose, self._channels_mult*filters, 6, **self._kwargs)(hidden) # (BxT, 30, 30, 32)
            out = self.get('out', tfkl.Conv2DTranspose, self._out_channels, 6, strides=2, activation=self._out_activation)(hidden) # (BxT, 64, 64, out_channels)
            out = tf.reshape( out, tf.concat([tf.shape(states)[:2], tf.shape(out)[1:]], -1) ) # (B, T, 64, 64, out_channels)
        return out

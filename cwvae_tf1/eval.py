...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seq-len", default=500, type=int, help="total length of evaluation sequences"
    )
    parser.add_argument("--data-root", default=None, type=str)
    parser.add_argument(
        "--num-samples", default=1, type=int, help="samples per example"
    )
    parser.add_argument(
        "--open-loop-ctx", default=36, type=int, help="length of context"
    )
    parser.add_argument(
        "--use-obs", default=None, type=str, help="string of T/Fs per level"
    )
    parser.add_argument(
        "--no-save-grid",
        action="store_true",
    ...
    # # DEBUG # TODO remove
    # args.model_dir = "./logs/minerl/minerl_cwvae_rssmcell_1l_f2_decsd0.4_enchl1_ences10_edchnlmult1_ss10_ds20_es20_seq40_lr0.0001_bs5/model_0/"
    # args.model_dir = "/media/vaibhav/Data1/Study/planning-project/hierarchical-rssm/git/cwvae/logs/mmnist/mmnist_cwvae_rssmcell_3l_f6_decsd1.0_enchl3_ences1000_edchnlmult1_ss20_ds200_es200_seq100_lr0.0003_bs10/model"
    ...
    # Set directories.
    exp_rootdir = str(pathlib.Path(args.model_dir).resolve().parent)
    ...
    # Load dataset.
    _, val_data_batch = load_dataset(cfg, hold_out_val=False)
    ...
    print("Restoring model from {}".format(args.model_dir))
    checkpoint.restore(session, os.path.basename(os.path.normpath(args.model_dir)))

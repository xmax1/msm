...
def stack_dictionaries(list_of_dicts, dim=1):
    new_dict = {}
    for k, v in list_of_dicts[0].items():
        new_list = [x[k] for x in list_of_dicts]
        stacked_tensor = tc.stack(new_list, dim=dim)
        new_dict[k] = stacked_tensor
    return new_dict
...
class CWVAE(nn.Module):
    ...
    def forward(self, data, use_observations=None, initial_state=None):
        ...
        if use_observations is None:
            use_observations = self.cfg.levels * [True]
        if initial_state is None:
            initial_state = self.cfg.levels * [None]

        cells = [RSSMCell(self.cfg) for _ in range(self.cfg.levels)]

        priors = []
        posteriors = []
        last_states = []
        top_level = self.cfg.levels - 1
        for level, (cell, use_obs, obs_inputs, initial) in \
            ...
            if level == top_level:  # is_top_level = True as top level is the max # level
                ...
                context = tc.unsqueeze(context, dim=2)
                context = tc.tile(context, [1, 1, self.cfg.tmp_abs_factor] + (len(context.shape) - 3) * [1])
                s = context.shape
            ...
            # state['sample'] = state['sample'][:, None, :]

            # SCAN FUNCTION, DOES IT WORK?? NOBODY KNOWS
            split_contexts = [x.squeeze(1) for x in context.split(1, dim=1)]
            split_obs_inputs = [x.squeeze(1) for x in obs_inputs.split(1, dim=1)]
            ...
            for c, o in zip(split_contexts, split_obs_inputs):
                ...
                unrolled_priors.append(prior)
                unrolled_posteriors.append(posterior)

            prior = stack_dictionaries(unrolled_priors, dim=1)
            posterior = stack_dictionaries(unrolled_posteriors, dim=1)
    ...
    def open_loop_unroll(self, inputs):
        ...
        assert self.cfg.open_loop_ctx % (self.cfg.tmp_abs_factor ** (self.cfg.levels - 1)) == 0, \
        ...
        ctx_lens = [self.cfg.open_loop_ctx // self.cfg.tmp_abs_factor ** level for level in range(self.cfg.levels)]
        ...
        last_states, _, _ = self(pre_inputs, use_observations=self.cfg.use_observations)
        ...
        _, predictions, _ = self(post_inputs, use_observations=self.cfg.levels * [False], initial_state=last_states)
        return predictions
...
class Model(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        cfg.enc_dense_features = self.encoder.enc_dense_features
        self.model = CWVAE(cfg)
    ...
    def forward(self, obs):
        '''
        obs: tensor (m, t, c, f, f)
        '''
        assert obs.shape[-3:] == (self.cfg.channels, 64, 64)
        encoded_obs = self.encoder(obs)
        _, priors, posteriors = self.model(encoded_obs)
        decoded_obs = self.decode(posteriors)
        output = tfd.Independent(tfd.Normal(decoded_obs, self.cfg.dec_stddev), -1)
        priors = [tfd.Independent(tfd.Normal(d["mean"], d["stddev"]), -1) for d in priors]
        posteriors = [tfd.Independent(tfd.Normal(d["mean"], d["stddev"]), -1) for d in posteriors]
        ...
        kls = [tc.mean(tc.distributions.kl.kl_divergence(posterior, prior), dim=0) for prior, posterior in zip(priors, posteriors)]

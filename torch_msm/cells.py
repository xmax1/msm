...
class RSSMPrior(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.relu = nn.ReLU(inplace=False)  # if true input is replaced by output in memory, uses less memory but only more efficient in some cases
...
class RSSMPosterior(nn.Module):
    ...
    def forward(self, prior, obs_inputs):
        ...
        sample = tc.normal(mean, stddev)
        output = tc.cat([sample, prior['det_out']], dim=-1)
...
class RSSMCell(nn.Module):
    ...
    @property
    def state_size(self):
        return dict(mean=self.cfg.cell_stoch_size, stddev=self.cfg.cell_stoch_size, \
            sample=self.cfg.cell_stoch_size, det_out=self.cfg.cell_deter_size, \
                det_state=self.cfg.cell_deter_size, output=self.cfg.cell_stoch_size + self.cfg.cell_deter_size)

...
class Encoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.conv_net = nn.Sequential(nn.Conv2d(cfg.channels, tf1, kernel_size=(4, 4), stride=(2, 2), padding='valid'), leaky_relu,
        ...
        self.enc_dense_features = self.conv_net(tc.rand((1, cfg.channels, 64, 64))).reshape(-1).shape[-1]
        self.enc_dense_layers = {i:nn.Sequential(*list(nn.Sequential(nn.Linear(self.enc_dense_features, self.enc_dense_features), nn.ReLU())
    ...
    def forward(self, obs):
        ...
        for level in range(1, self.cfg.levels):
            ...
            layer = tc.nn.functional.pad(layer, (0, 0, 0, timesteps_to_pad, 0, 0))  # ((before, after), ... ), also pads with zeros by default
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.dense1 = nn.Linear(cfg.cell_stoch_size + cfg.cell_embed_size, cfg.channels_mult * 1024)  # channels_mult is 1 and this code only works if it is 1 not my fault

        tf3, tf2, tf1 = cfg.total_filters, cfg.total_filters * 2, cfg.total_filters * 4
        self.convt = nn.Sequential(nn.ConvTranspose2d(1024, tf1, kernel_size=5, stride=2), leaky_relu,
                                   nn.ConvTranspose2d(tf1, tf2, kernel_size=5, stride=2), leaky_relu,
                                   nn.ConvTranspose2d(tf2, tf3, kernel_size=6, stride=2), leaky_relu,
                                   nn.ConvTranspose2d(tf3, cfg.channels, kernel_size=6, stride=2), nn.Tanh())
    ...
    def forward(self, bottom_layer_output):
        ...
        """
        print(bottom_layer_output.shape)
        x = self.dense1(bottom_layer_output)
        print(x.shape)
        x = x.reshape((-1, x.shape[-1], 1, 1))  # (BxT, 1, 1, 1024)
        print(x.shape)
        x = self.convt(x)

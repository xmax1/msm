...
class Config:
    ...
    datadir: str=r"C:\Users\amwil\OneDrive\projects\msm\cwvae-jax\datasets\data"  # Path to root data directory
...
def parse_config(eval=False):
    '''
    - Take the default values from the Config class
    - Initialize a Config class object with those default values
    - Replace the default values with values taken from yml file (required because some defaults not filled)
    '''
    p = ArgumentParser()  # argument parser object 
    for f in fields(Config): 
        kwargs = (  # these two lines distinguish between boolean and string args
            dict(action='store_true') if f.type is bool and not f.default else  # if there is not a default value
            dict(default=f.default, type=f.type))  
        p.add_argument(f'--{f.name}', **kwargs, **f.metadata)
    c = Config(**vars(p.parse_args()))  # vars(p.parse_args()) returns a dictionary of arguments
...
if __name__ == '__main__':
    ...
    # for f in fields(Config):
        # print(f.name, not f.default, f.default)
